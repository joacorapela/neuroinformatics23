...
def main(argv):
    ...
    parser.add_argument("--start_time", type=float,
                        help="LFP extraction start time (sec)", default=4800.0)
    parser.add_argument("--duration", type=float,
                        help="LFP extraction duration (sec)", default=600.0)
    ...
    parser.add_argument("--segment_len", type=int,
                        help="Segement length (samples)", default=2**10)
    ...
    parser.add_argument("--fig_filename_pattern", type=str,
                        ...
                        default="../../figures/comodugram_segmentLength_{:d}_pid_{:s}_channel_{:d}.{:s}")
    ...
    zmin = np.min(comodugram)
    zmax = np.max(comodugram)
...
#             hovertext[-1].append(f"time: {xx}<br />frequency: {yy}<br />Sxx: {Sxx[yi][xi]}<br />")
    ...
    title = f"Comodugram: channel {channel_nro}, region {channel_locs_acronyms[channel_nro]}, segment_len {segment_len}"
    fig = go.Figure()
    trace = go.Heatmap(x=f, y=f, z=comodugram, colorscale=colorscale, zmin=zmin, zmax=zmax)
    fig.add_trace(trace)
