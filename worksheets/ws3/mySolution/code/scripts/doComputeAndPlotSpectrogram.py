...
def main(argv):
    ...
    parser.add_argument("--fig_filename_pattern", type=str,
                        ...
                        default="../../figures/spectrogram_startTime_{:.02f}_duration_{:.02f}_segmentLength_{:d}_pid_{:s}_channel_{:d}.{:s}")
...
#             hovertext[-1].append(f"time: {xx}<br />frequency: {yy}<br />Sxx: {Sxx[yi][xi]}<br />")
    ...
    fig = go.Figure()
    trace = go.Heatmap(x=start_time+t, y=f, z=np.log10(Sxx),
...
#                        hoverinfo="text", text=hovertext,
    ...
    fig.write_image(fig_filename_pattern.format(start_time, duration, segment_len, pid, channel_nro, "png"))
    fig.write_html(fig_filename_pattern.format(start_time, duration, segment_len, pid, channel_nro, "html"))
