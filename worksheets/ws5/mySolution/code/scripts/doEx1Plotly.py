...
def main(argv):
    ...
    # no zmax
    fig = utils.getHeatmap(xs=times, ys=clusters_ids, zs=activity_arrayZ.T,
                           hovertext=hovertext, zmin=zmin,
                           zmax=np.max(activity_arrayZ),
                           x_label=x_label, y_label=y_label)
    ...
    # no zscored
    fig = utils.getHeatmap(xs=times, ys=clusters_ids, zs=activity_array.T,
                           hovertext=hovertext, zmin=zmin, zmax=zmax,
                           x_label=x_label, y_label=y_label)
    fig.write_image(fig_filename_pattern.format(bin_size, "noZscored",  "png"))
    fig.write_html(fig_filename_pattern.format(bin_size, "noZscored",  "html"))
