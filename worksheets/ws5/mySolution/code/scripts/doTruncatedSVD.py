...
def main(argv):
    ...
    parser.add_argument("--n_svd_components", type=int,
    ...
    activity_arrayZ_truncated = truncate_svd(u=u, s=s, vh=vh,
                                             n_components=n_components)

    plt.figure(figsize=(fig_width, fig_height))
    shw = plt.pcolor(X, Y, activity_arrayZ_truncated[:, argsort_res].T, norm=norm)
    ...
    extent = (times.min(), times.max(), clusters_ids.min(), clusters_ids.max())
    argsort_res = np.argsort(vh[0, :])

    plt.figure(figsize=(fig_width, fig_height))
    plt.imshow(activity_arrayZ[:, argsort_res].T, aspect='auto',
               vmin=zmin, vmax=zmax, extent=extent)
    plt.xlabel(x_label)
    plt.ylabel(y_label)
    plt.savefig(fig_filename_pattern.format(bin_size, "vh0Sorted",  "png"))
    # plt.show()
